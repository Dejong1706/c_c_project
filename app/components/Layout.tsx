...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Top nav */}
      <header style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', height: '60px', display: 'flex', alignItems: 'center', padding: '0 20px', position: 'sticky', top: 0, zIndex: 50, gap: '8px' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none', marginRight: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-1)', lineHeight: 1 }}>BuildCalc</span>
            <span style={{ fontSize: '11px', color: 'var(--text-3)', lineHeight: 1 }}>Free construction calculators</span>
          </div>
        </Link>
        ...
        <button
          ...
          style={{ display: 'none', background: 'none', border: '1px solid var(--border)', borderRadius: '8px', padding: '6px 8px', cursor: 'pointer', color: 'var(--text-2)', lineHeight: 1 }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        {/* 데스크탑 사이드바 */}
        <aside className="desktop-sidebar" style={{ width: '224px', background: 'var(--surface)', borderRight: '1px solid var(--border)', position: 'sticky', top: '60px', height: 'calc(100vh - 60px)', overflowY: 'auto', flexShrink: 0 }}>
          {sidebarContent}
        </aside>
        ...
        {mobileOpen && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 40, display: 'flex' }}>
            <div style={{ background: 'var(--surface)', width: '260px', borderRight: '1px solid var(--border)', overflowY: 'auto', paddingTop: '60px' }}>
              {sidebarContent}
            </div>
        ...
        {/* 메인 콘텐츠 */}
        <main className="main-content" style={{ flex: 1, minWidth: 0, width: '100%' }}>
          {children}
        </main>
      </div>

      <footer style={{ background: 'var(--surface)', borderTop: '1px solid var(--border)', padding: '20px 28px', fontSize: '12px', color: 'var(--text-3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
