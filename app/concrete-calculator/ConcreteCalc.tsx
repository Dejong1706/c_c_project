...
export default function ConcreteCalc() {
  ...
  const [diamUnit, setDiamUnit] = useState<UnitLen>("m");
  ...
  const results = useMemo(() => {
    ...
    const netVol = type === 4 ? Math.PI * R * R * D : L * W * D;
    ...
    <>
      {!isRound ? (
        ...
        <Field
          ...
          placeholder="e.g. 0.5"
          units={["m", "ft", "cm", "in"]}
          selectedUnit={diamUnit}
          onUnitChange={(v) => setDiamUnit(v as UnitLen)}
        />
      )}
      <Field
        label={type === 0 ? "Thickness" : "Height / Depth"}
        id="depth"
        value={depth}
        onChange={setDepth}
